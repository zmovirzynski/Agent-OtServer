...
CombatDamage Combat::getCombatDamage(Creature* creature, Creature* target) const
{
	...
	if (Pokemon* pokemon = creature->getPokemon()) {
		if (params.valueCallback) {
			params.valueCallback->getMinMaxValues(pokemon, damage, params.power);
		}
	}
	return damage;
}
...
bool Combat::isProtected(const Player* attacker, const Player* target)
{
	...
	if (!attacker->getProfession()->allowsPvp() || !target->getProfession()->allowsPvp()) {
		return true;
	}
...
bool Combat::setParam(CombatParam_t param, uint32_t value)
{
	switch (param) {
		...
		case COMBAT_PARAM_POWER: {
			params.power = static_cast<uint16_t>(value);
			return true;
		}
	}
	return false;
...
void Combat::doTargetCombat(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	result = g_game.combatChangeHealth(caster, target, damage);
...
void Combat::doAreaCombat(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
	...
	for (Creature* creature : toDamageCreatures) {
		...
		result = g_game.combatChangeHealth(caster, creature, damageCopy);
...
void ValueCallback::getMinMaxValues(Pokemon* pokemon, CombatDamage& damage, uint16_t power) const
{
	//onGetPlayerMinMaxValues(...)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - ValueCallback::getMinMaxValues] Call stack overflow" << std::endl;
