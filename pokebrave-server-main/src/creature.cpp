...
void Creature::onThink(uint32_t interval)
{
	...
	g_moduleCallback->executeOnThink(this, interval);
}
...
void Creature::onCreatureDisappear(const Creature* creature, bool isLogout)
{
	...
	if (Player* player = getPlayer()) {
		g_moduleCallback->executeOnDisappear(player, creature);
	}
}
...
bool Creature::dropCorpse(Creature* lastHitCreature, Creature* mostDamageCreature, bool lastHitUnjustified, bool mostDamageUnjustified)
{
	...
	if (!lootDrop && pokemon) {
		if (master) {
			g_moduleCallback->executeOnDeath(this, nullptr, lastHitCreature, mostDamageCreature, lastHitUnjustified, mostDamageUnjustified);
		}
		...
		if (corpse) {
			...
			corpse->setCorpseName(getName());
			if (pokemon) {
				corpse->setPokemonCorpseLevel(pokemon->getLevel());
				corpse->setPokemonCorpseGender(pokemon->getGender());
				corpse->setPokemonCorpseNature(pokemon->getNature());
				corpse->setPokemonCorpseIvs(pokemon->getIvs());
			}
		}

		g_moduleCallback->executeOnDeath(this, corpse, lastHitCreature, mostDamageCreature, lastHitUnjustified, mostDamageUnjustified);
...
void Creature::onTickCondition(ConditionType_t type, bool& bRemove)
{
	...
	switch (type) {
		...
		case CONDITION_ENERGY:
			bRemove = (field->getCombatType() != COMBAT_ELETRICDAMAGE);
			break;
...
bool Creature::onKilledCreature(Creature* target, bool)
{
	...
	g_moduleCallback->executeOnKill(this, target);
	return false;
}
...
bool Creature::addTag(const std::string& name, const std::string& value)
{
	if (hasTag(name)){
		std::cerr << "Error: Creature::addTag -> The tag " << name << " already exists.\n";
		return false;
	}
...
bool Creature::addTimedTag(const std::string& name, uint32_t duration, const std::string& value = "")
{
	if (!addTag(name, value)) {
		return false;
	}

	uint32_t eventId = g_scheduler.addEvent(createSchedulerTask(duration, std::bind(&Game::removeCreatureTag, &g_game, getID(), name)));
	timedTags.insert(std::make_pair(name, eventId));
	return true;
}

bool Creature::setTag(const std::string& name, const std::string& value)
{
	if (hasTag(name)) {
		tags[name] = value;
		return true;
...
bool Creature::removeTag(const std::string& name)
{
	if (!hasTag(name)) {
		std::cerr << "Warning: Creature::addTag -> The tag " << name << " don't exists.\n";
		return false;
	}

	tags.erase(name);
	if (timedTags.find(name) != timedTags.end()) {
		uint32_t eventId = timedTags[name];
		g_scheduler.stopEvent(eventId);
		timedTags.erase(name);
	}
	return true;
}

bool Creature::hasTag(const std::string& name)
{
...
std::string Creature::getTag(const std::string& name)
{
	if (!hasTag(name)) {
		return "";
	}
...
void Creature::addEventCheckFollow()
{
	if (eventCheckFollow != 0) {
		return;
	}

	eventCheckFollow = g_scheduler.addEvent(createSchedulerTask(EVENT_CHECKFOLLOW, std::bind_front(&Game::eventAddToCheckFollow, &g_game, getID())));
}

void Creature::stopEventCheckFollow()
{
	if (eventCheckFollow != 0) {
		g_scheduler.stopEvent(eventCheckFollow);
		eventCheckFollow = 0;
	}
}
