...
static constexpr int32_t EVENT_CHECKFOLLOW = 75;
...
class Game
{
	public:
		...
		void checkLight();
		void removeCreatureTag(uint32_t creatureId, const std::string& tagName);
		...
		void addCreatureJump(const Creature* target, uint16_t height, uint16_t duration);
		static void addCreatureJump(const SpectatorVec& spectators, const Creature* target, uint16_t height, uint16_t duration);
		...
		void eventAddToCheckFollow(uint32_t creatureId);
