...
void Game::start(ServiceManager* manager)
{
	...
	g_scheduler.addEvent(createSchedulerTask(EVENT_CHECKFOLLOW, std::bind_front(&Game::checkFollow, this, false)));
}
...
Creature* Game::getCreatureByName(const std::string& s)
{
	...
	{
		auto it = std::find_if(pokemons.begin(), pokemons.end(), equalCreatureName);
		if (it != pokemons.end()) {
			return it->second;
		}
	}
...
bool Game::internalPlaceCreature(Creature* creature, const Position& pos, bool extendedPos /*=false*/, bool forced /*= false*/)
{
	...
	{
		pokemon->generateIvs();
		pokemon->generateGender();
		pokemon->generateNature();
		pokemon->generateLevel();
		pokemon->updateMaxHealth();
		pokemon->setHealth(pokemon->getMaxHealth());
		pokemon->updateSpeed();
		g_game.addCreatureHealth(pokemon);

		PokemonType* pType = g_pokemons.getPokemonType(pokemon->getName());
		if (pType) {
			pokemon->setBaseStats(pType->baseStats);
			for (auto& [tagName, value] : pType->tags) {
				pokemon->addTag(tagName, value);
			}
		}
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags /*= 0*/)
{
	...
	if (!toTile || (toTile->getGround() && toTile->getGround()->getID() == INVISIBLE_TILE_ID)) {
		if (!player) 
		...
		for (int i = 0; i < FLY_TILES_RANGE; i++) {
			for (int j = 0; j < FLY_TILES_RANGE; j++) {
				Position newPos = destPos;
				newPos.x += (i - 1);
				newPos.y += (j - 1);

				Tile* tmpTile = map.getTile(newPos.x, newPos.y, newPos.z);
				Item* tmpGround = tmpTile ? tmpTile->getGround() : nullptr;

				if (tmpTile && tmpGround) {
					continue;
				}

				if (!tmpTile) {
					tmpTile = new DynamicTile(newPos.x, newPos.y, newPos.z);
					tmpTile->setGround(Item::CreateItem(INVISIBLE_TILE_ID));
					map.setTile(newPos, tmpTile);
				...
				SpectatorVec spectators;
				map.getSpectators(spectators, newPos, true, true);

				for (Creature* spectator : spectators) {
					if (Player* tmpPlayer = spectator->getPlayer()) {
						tmpPlayer->sendUpdateTile(tmpTile, newPos);
					}
				}
			}
		}
	...
	if (toTile && toTile->getGround() == nullptr && player) {
		if (player->isFlying()) {
			toTile->setGround(Item::CreateItem(INVISIBLE_TILE_ID));

			SpectatorVec spectators;
			map.getSpectators(spectators, destPos, true, true);

			for (Creature* spectator : spectators) {
				if (Player* tmpPlayer = spectator->getPlayer()) {
					tmpPlayer->sendUpdateTile(toTile, destPos);
				}
			}
		}
	}
                          ...
                          uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	...
	Item* toCylinderGround = toCylinderTile ? toCylinderTile->getGround() : nullptr;

	if (toCylinderGround && toCylinderGround->getID() == INVISIBLE_TILE_ID) {
		player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
		return;
	}
	...
	if (!toTile) {
		if (Creature* creature = thing->getCreature()) {
			if (Player* player = creature->getPlayer()) {
				if (player->isFlying()) {
					Tile* newTile = new DynamicTile(newPos.x, newPos.y, newPos.z);
					newTile->setGround(Item::CreateItem(INVISIBLE_TILE_ID));
					...
					for (Creature* spectator : spectators) {
						if (Player* tmpPlayer = spectator->getPlayer()) {
							tmpPlayer->sendUpdateTile(newTile, newPos);
						}
					}

					toTile = map.getTile(newPos);
...
void Game::playerWriteItem(uint32_t playerId, uint32_t windowTextId, const std::string& text)
{
	...
	if (!g_moduleCallback->executeOnTextEdit(player, writeItem, text)) {
		return;
	}
...
std::string Game::getTradeErrorDescription(ReturnValue ret, Item* item)
{
	if (item) {
		if (ret == RETURNVALUE_NOTENOUGHCAPACITY) {
			...
			return fmt::format("You can only have 6 Pokemon.");
		}
	}
...
void Game::playerLookAt(uint32_t playerId, const Position& pos, uint8_t stackPos)
{
	...
	g_moduleCallback->executeOnLook(player, pos, thing, stackPos, lookDistance);
}
                               ...
                               bool ghostMode, SpectatorVec* spectatorsPtr/* = nullptr*/, const Position* pos/* = nullptr*/, bool echo/* = false*/)
{
	...
	g_moduleCallback->executeOnSay(creature, type, text);
	return true;
}
...
void Game::checkFollow(bool thread)
{
	...
	if (!thread) {
		...
		g_jobsScheduler.runTask(std::bind_front(&Game::checkFollow, this, true));
...
void Game::eventAddToCheckFollow(uint32_t creatureId)
{
	Creature* creature = getCreatureByID(creatureId);
	if (creature && !creature->isRemoved()) {
		creature->eventCheckFollow = 0;
		addToCheckFollow(creature);
	}
}
...
void Game::changeSpeed(Creature* creature, int32_t varSpeedDelta)
{
	...
	if (Pokemon* pokemon = creature->getPokemon()) {
		varSpeed = MIN_POKEMON_SPEED_VALUE + varSpeedDelta;
	} else {
		varSpeed = creature->getSpeed() - creature->getBaseSpeed();
...
void Game::combatGetTypeInfo(CombatType_t combatType, Creature* target, TextColor_t& color, uint8_t& effect)
{
	switch (combatType) {
		...
		case COMBAT_FAIRYDAMAGE: {
			color = TEXTCOLOR_ORCHID;
			effect = CONST_ME_NONE;
			break;
		}
		case COMBAT_FIGHTINGDAMAGE: {
			color = TEXTCOLOR_SADDLEBROWN;
			effect = CONST_ME_NONE;
			break;
		}
		case COMBAT_STEELDAMAGE: {
			color = TEXTCOLOR_SILVER;
			effect = CONST_ME_NONE;
			break;
		}
		case COMBAT_FLYINGDAMAGE: {
			color = TEXTCOLOR_DARKKHAKI;
			effect = CONST_ME_NONE;
			break;
		}
		...
		case COMBAT_GHOSTDAMAGE: {
			color = TEXTCOLOR_DARKMAGENTA;
			effect = CONST_ME_NONE;
			break;
		}
		case COMBAT_DRAGONDAMAGE: {
			color = TEXTCOLOR_LIGHTSTEELBLUE;
			effect = CONST_ME_NONE;
			break;
		}
		case COMBAT_GROUNDDAMAGE: {
			color = TEXTCOLOR_SIENNA;
			effect = CONST_ME_NONE;
			break;
		}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (damage.origin != ORIGIN_NONE) {
			g_moduleCallback->executeOnHealthChange(target, attacker, damage);
		...
		if (realDamage >= targetHealth) {
			if(!g_moduleCallback->executeOnPrepareDeath(target, attacker)){
				return false;
			}
		}
...
void Game::addCreatureJump(const Creature* target, uint16_t height, uint16_t duration)
{
	SpectatorVec spectators;
	map.getSpectators(spectators, target->getPosition(), true, true);
	addCreatureJump(spectators, target, height, duration);
}

void Game::addCreatureJump(const SpectatorVec& spectators, const Creature* target, uint16_t height, uint16_t duration)
{
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			tmpPlayer->sendCreatureJump(target, height, duration);
		}
	}
}
...
void Game::removeCreatureTag(uint32_t creatureId, const std::string& tagName)
{
	Creature* creature = getCreatureByID(creatureId);
	if (!creature || creature->getHealth() < 0) {
		return;
	...
	if (creature->timedTags.find(tagName) != creature->timedTags.end()) {
		creature->timedTags.erase(tagName);
	}

	creature->removeTag(tagName);
}
...
void Game::updatePokemon(Pokemon* pokemon) {

	SpectatorVec spectators;
	map.getSpectators(spectators, pokemon->getPosition(), true, true);
	for (Creature* spectator : spectators) {
		spectator->getPlayer()->sendUpdatePokemon(pokemon);
	}
}
...
void Game::parsePlayerExtendedOpcode(uint32_t playerId, uint8_t opcode, const std::string& buffer)
{
	...
	g_moduleCallback->executeOnExtendedOpcode(player, opcode, buffer);
}
...
void Game::playerAnswerModalWindow(uint32_t playerId, uint32_t modalWindowId, uint8_t button, uint8_t choice)
{
	...
	g_moduleCallback->executeOnModalWindow(player, modalWindowId, button, choice);
}
