...
uint16_t Item::getPokemonCount() const
{
	ItemType& it = Item::items.getItemType(this->getID());
	return it.isPokeball() ? 1 : 0;
}
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.armor != 0 || (item && item->getArmor() != 0) || it.showAttributes) {
		bool begin = true;
