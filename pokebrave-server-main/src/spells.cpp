...
{
	Spell* spell = getInstantSpellByName(name);
	if (!spell) {
		return nullptr;
	}
	return spell;
...
bool Spell::configureSpell(const pugi::xml_node& node)
{
	...
	for (auto professionNode : node.children()) {
		...
		int32_t professionId = g_professions.getProfessionId(attr.as_string());
		if (professionId != -1) {
			attr = professionNode.attribute("showInDescription");
			vocSpellMap[professionId] = !attr || attr.as_bool();
		} else {
			std::cout << "[Warning - Spell::configureSpell] Wrong profession name: " << attr.as_string() << std::endl;
		}
	}
	return true;
}
...
bool Spell::playerSpellCheck(Player* player) const
{
	...
	} else if (!vocSpellMap.empty() && vocSpellMap.find(player->getProfessionId()) == vocSpellMap.end()) {
		player->sendCancelMessage(RETURNVALUE_YOURPROFESSIONCANNOTUSETHISSPELL);
		g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
		return false;
	}
...
void Spell::postCastSpell(Player* player, bool finishedCast /*= true*/) const
{
	if (finishedCast) {
...
bool InstantSpell::executeCastSpell(Creature* creature, const LuaVariant& var)
{
	...
	if (Pokemon* pokemon = creature->getPokemon()) {
		...
		g_game.internalCreatureSay(pokemon, TALKTYPE_POKEMON_SAY, getName(), false, &spectators, &pokemon->getPosition(), false);

		if (Pokeball* pokeball = pokemon->getPokeball()) {
			pokeball->resetMoveCooldown(asLowerCaseString(getName()));
		}
	}
	return scriptInterface->callFunction(2);
}
...
bool InstantSpell::canCast(const Player* player) const
{
	...
	if (isLearnable()) {
		...
		if (vocSpellMap.empty() || vocSpellMap.find(player->getProfessionId()) != vocSpellMap.end()) {
			return true;
		}
	}
