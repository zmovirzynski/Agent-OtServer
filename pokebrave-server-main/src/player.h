...
class Player final : public Creature, public Cylinder
{
	public:
		...
		uint16_t getPokemonCapacity() const {
			if (hasFlag(PlayerFlag_CannotPickupItem)) {
				return 0;
		...
		uint16_t getFreePokemonCapacity() const {
			if (hasFlag(PlayerFlag_CannotPickupItem)) {
				return 0;
			} else if (hasFlag(PlayerFlag_HasInfinitePokemonCapacity)) {
				return std::numeric_limits<uint16_t>::max();
			} else {
				return std::max<uint16_t>(0, 6 - inventoryPokemonCount);
			}
		}
		...
		void sendCreatureJump(const Creature* creature, uint16_t time, uint16_t height) const {
			if (client) {
				client->sendCreatureJump(creature, time, height);
			}
		}
		...
		void sendUpdatePokemon(Pokemon* pokemon) {
			if (client) {
				client->sendUpdatePokemon(pokemon);
			}
		}
	...
	private:
		...
		Item* orderItem = nullptr;
		Item* pokedexItem = nullptr;
		...
		uint16_t inventoryPokemonCount = 0;
