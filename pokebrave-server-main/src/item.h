...
class ItemAttributes
{
	...
	private:
		...
		const static uint32_t intAttributeTypes = ITEM_ATTRIBUTE_ACTIONID | ITEM_ATTRIBUTE_UNIQUEID | ITEM_ATTRIBUTE_DATE
			...
			| ITEM_ATTRIBUTE_ATTACK_SPEED | ITEM_ATTRIBUTE_POKEMONCORPSELEVEL | ITEM_ATTRIBUTE_POKEMONCORPSEGENDER | ITEM_ATTRIBUTE_POKEMONCORPSENATURE;
		const static uint32_t stringAttributeTypes = ITEM_ATTRIBUTE_DESCRIPTION | ITEM_ATTRIBUTE_TEXT | ITEM_ATTRIBUTE_WRITER
			| ITEM_ATTRIBUTE_NAME | ITEM_ATTRIBUTE_ARTICLE | ITEM_ATTRIBUTE_PLURALNAME | ITEM_ATTRIBUTE_CORPSENAME;
...
class Item : virtual public Thing
{
	public:
		...
		void setCorpseName(const std::string& name) {
			setStrAttr(ITEM_ATTRIBUTE_CORPSENAME, name);
		}
		const std::string& getCorpseName() const {
			if (!attributes) {
				return ItemAttributes::emptyString;
			}
			return getStrAttr(ITEM_ATTRIBUTE_CORPSENAME);
		}
		...
		virtual uint16_t getPokemonCount() const;
		...
		void setPokemonCorpseLevel(uint32_t level) {
			setIntAttr(ITEM_ATTRIBUTE_POKEMONCORPSELEVEL, level);
		}
		uint32_t getPokemonCorpseLevel() const{
			return getIntAttr(ITEM_ATTRIBUTE_POKEMONCORPSELEVEL);
		}

		void setPokemonCorpseGender(uint8_t gender) {
			setIntAttr(ITEM_ATTRIBUTE_POKEMONCORPSEGENDER, gender);
		}
		uint8_t getPokemonCorpseGender() const {
			return getIntAttr(ITEM_ATTRIBUTE_POKEMONCORPSEGENDER);
		}

		void setPokemonCorpseNature(uint8_t nature) {
			setIntAttr(ITEM_ATTRIBUTE_POKEMONCORPSENATURE, nature);
		}
		uint8_t getPokemonCorpseNature() const {
			return getIntAttr(ITEM_ATTRIBUTE_POKEMONCORPSENATURE);
		}

		void setPokemonCorpseIvs(PokemonValues& ivs) {
			this->ivs = ivs;
		}
		PokemonValues& getPokemonCorpseIvs(){
			return ivs;
		}
		...
		bool isCuttable() const {
			return items[id].cuttable;
		}
		bool isSmashable() const {
			return items[id].smashable;
		}
		bool isDiggable() const {
			return items[id].diggable;
		}
