...
class Creature : virtual public Thing
{
	...
	public:
		...
		virtual PokemonType_t getFirstType() const {
			return POKEMON_TYPE_NONE;
		}
		virtual PokemonType_t getSecondType() const {
			return POKEMON_TYPE_NONE;
		}
		...
		void setHealth(int32_t health) {
			this->health = std::min<int32_t>(health, this->healthMax);
		}
		...
		void setMaxHealth(int32_t health) {
			...
			this->health = std::min<int32_t>(this->health, this->healthMax);
		}
		...
		void goToFollowCreatureContinue();
		...
		bool addTimedTag(const std::string& name, uint32_t duration, const std::string& value);
		bool setTag(const std::string& name, const std::string& value);
		...
		void addEventCheckFollow();
		void stopEventCheckFollow();
	...
	protected:
		...
		std::map<std::string, uint32_t> timedTags;
