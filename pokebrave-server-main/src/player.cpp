...
Player::Player(ProtocolGame_ptr p) :
	Creature(), lastPing(OTSYS_TIME()), lastPong(lastPing), inbox(new Inbox(ITEM_STORE_INBOX)), storeInbox(new StoreInbox(ITEM_STORE_INBOX)), client(std::move(p))
{
	...
	orderItem = new Item(ITEM_ORDER);
	pokedexItem = new Item(ITEM_POKEDEX);
}
...
{
	int32_t value{ -1 };
	getStorageValue(PLAYER_STORAGE_FLY, value);
...
{
	if (!flying) {
		Tile* tmpTile = g_game.map.getTile(getPosition());
		Item* tmpGround = tmpTile ? tmpTile->getGround() : nullptr;

		if (tmpTile && tmpGround && tmpGround->getID() == INVISIBLE_TILE_ID) {
			return;
		}
	}

	int32_t value = flying ? 1 : -1;
	addStorageValue(PLAYER_STORAGE_FLY, value);
}

void Player::removeLastFlyTiles()
{
	if (isFlying()) {
		const Position& lastPos = getLastPosition();
		const Position& currentPos = getPosition();

		int offsetX = 1;
		int offsetY = lastPos.z == currentPos.z ? 1 : 0;
		...
		for (int i = 0; i < FLY_TILES_RANGE; i++) {
			for (int j = 0; j < FLY_TILES_RANGE; j++) {
				...
				Tile* tmpTile = g_game.map.getTile(newPos);
				Item* tmpGround = tmpTile ? tmpTile->getGround() : nullptr;

				if (tmpTile && tmpGround && tmpGround->getID() == INVISIBLE_TILE_ID) {
					const Position& tmpTilePos = tmpTile->getPosition();

					if (currentPos.z != tmpTilePos.z || Position::getDistance(currentPos, tmpTilePos) > 1) {
						if (!tmpTile->getTopCreature()) {
							if (!tmpTile->getTopTopItem()) {
								g_game.map.removeTile(tmpTilePos);
							} else {
								g_game.internalRemoveItem(tmpGround);
								tmpTile->setGround(nullptr);

								SpectatorVec spectators;
								g_game.map.getSpectators(spectators, tmpTilePos, true, true);

								for (Creature* spectator : spectators) {
									if (Player* tmpPlayer = spectator->getPlayer()) {
										tmpPlayer->sendUpdateTile(tmpTile, tmpTilePos);
									}
								}
							}
						}
...
void Player::sendCreatureAppear(const Creature* creature, const Position& pos, bool isLogin)
{
	if (client) {
		client->sendAddCreature(creature, pos, creature->getTile()->getClientIndexOfCreature(this, creature), isLogin);
	}
	g_moduleCallback->executeOnAppear(this, creature);
}
...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	if (lookDistance == -1) {
		...
		if (group->access) {
			...
			s << " You are " << profession->getVocDescription() << '.';
		} else {
		...
		if (group->access) {
			...
			s << " is " << profession->getVocDescription() << '.';
		} else {
			s << " has no profession.";
		}
	}
...
void Player::addSkillAdvance(skills_t skill, uint64_t count)
{
	...
	while ((skills[skill].tries + count) >= nextReqTries) {
		...
		g_moduleCallback->executeOnAdvance(this, skill, (skills[skill].level - 1), skills[skill].level);
...
void Player::removeSkillTries(skills_t skill, uint64_t count, bool notify/* = false*/)
{
	...
	while (count > skills[skill].tries) {
		...
		skills[skill].tries = profession->getReqSkillTries(skill, skills[skill].level);
		skills[skill].level--;
	}

	skills[skill].tries = std::max<int32_t>(0, skills[skill].tries - count);
	skills[skill].percent = Player::getPercentLevel(skills[skill].tries, profession->getReqSkillTries(skill, skills[skill].level));
...
void Player::sendPing()
{
	...
	int32_t noPongKickTime = profession->getNoPongKickTime();
	...
	if (noPongTime >= noPongKickTime) {
		...
		if (!g_moduleCallback->executeOnLogout(this)) {
			return;
		}
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	if (prevLevel != level) {
		...
		g_moduleCallback->executeOnAdvance(this, SKILL_LEVEL, prevLevel, level);
...
{
	g_moduleCallback->executeOnLogout(this);
...
bool Player::hasPokemonCapacity(const Item* item) const
{
	...
	uint16_t pokemonCount = item->getContainer() != nullptr ? item->getPokemonCount() : 0;
	...
	switch (index) {
		...
		case CONST_SLOT_LEFT: {
			if (slotPosition & SLOTP_LEFT) {
				if (!g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
					if (inventory[CONST_SLOT_RIGHT] && (slotPosition & SLOTP_TWO_HAND)) {
						ret = RETURNVALUE_BOTHHANDSNEEDTOBEFREE;
					} else {
						ret = RETURNVALUE_NOERROR;
					}
...
{
	if (!g_moduleCallback->executeOnLogin(this)) {
		kickPlayer(true);
	}
}
