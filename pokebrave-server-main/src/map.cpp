...
void Map::removeTile(uint16_t x, uint16_t y, uint8_t z)
{
	...
	QTreeLeafNode* leaf = QTreeNode::getLeafStatic<QTreeLeafNode*, QTreeNode*>(&root, x, y);
	if (!leaf) {
		return;
	}
	...
	if (tile) {
		if (const CreatureVector* creatures = tile->getCreatures()) {
			for (int32_t i = creatures->size(); --i >= 0;) {
				if (Player* player = (*creatures)[i]->getPlayer()) {
					g_game.internalTeleport(player, player->getTown()->getPokemonCenterPosition(), false, FLAG_NOLIMIT);
				} else {
		...
		for (Creature* spectator : spectators) {
			if (Player* tmpPlayer = spectator->getPlayer()) {
				tmpPlayer->sendUpdateTile(tile, tilePos);
			}
		}
...
bool Map::placeCreature(const Position& centerPos, Creature* creature, bool extendedPos/* = false*/, bool forceLogin/* = false*/)
{
	...
	if (!tile) {
		...
		if(player) {
			if (player->isFlying()) {
				Tile *newTile = new DynamicTile(centerPos.x, centerPos.y, centerPos.z);
				newTile->setGround(Item::CreateItem(INVISIBLE_TILE_ID));
				setTile(centerPos, newTile);

				SpectatorVec spectators;
				getSpectators(spectators, centerPos, true, true);

				for (Creature* spectator : spectators) {
					if (Player* tmpPlayer = spectator->getPlayer()) {
						tmpPlayer->sendUpdateTile(newTile, centerPos);
					}
				}

				tile = getTile(centerPos);
	...
	tile = getTile(centerPos.x, centerPos.y, centerPos.z);
