...
void NetworkMessage::addItem(const Item* item)
{
	...
	if (g_config.getBoolean(ConfigManager::POKEBALL_FEATURE_PORTRAIT)) {
		if (const Pokeball* pokeball = item->getPokeball()) {
			addByte(1);
			add<uint8_t>(pokeball->getState());
			add<uint32_t>(pokeball->getPortrait());
		}
		else {
			addByte(0);
		}
