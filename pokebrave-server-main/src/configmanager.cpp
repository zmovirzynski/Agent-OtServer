...
bool ConfigManager::load()
{
	...
	if (!loaded) { //info that must be loaded one time (unless we reset the modules involved)
		...
		integer[CREATE_CHARACTER_PORT] = getGlobalNumber(L, "createCharacterPort", 7173);
		integer[CREATE_ACCOUNT_PORT] = getGlobalNumber(L, "createAccountProtocolPort", 7174);
	...
	boolean[FORCE_POKEMONTYPE_LOAD] = getGlobalBoolean(L, "forcePokemonTypesOnLoad", true);
	...
	// Pokemons System
	boolean[POKEMON_SYSTEM_LEVEL] = getGlobalBoolean(L, "enablePokemonLevel", false);
	boolean[POKEMON_SYSTEM_GENDER] = getGlobalBoolean(L, "enablePokemonGender", false);
	boolean[POKEMON_SYSTEM_BOOST] = getGlobalBoolean(L, "enablePokemonBoost", false);

	// Pokemons Features
	boolean[POKEMON_FEATURE_LEVEL] = getGlobalBoolean(L, "enableFeaturePokemonLevel", false);
	boolean[POKEMON_FEATURE_GENDER] = getGlobalBoolean(L, "enableFeaturePokemonGender", false);
	boolean[POKEMON_FEATURE_BOOST] = getGlobalBoolean(L, "enableFeaturePokemonBoost", false);
	boolean[POKEBALL_FEATURE_PORTRAIT] = getGlobalBoolean(L, "enableFeaturePokeballPortrait", false);
	...
	string[MOD_ENVIROMENT] = getGlobalString(L, "modularityEnviroment", "prod");
	string[LFS] = getGlobalString(L, "luaFileSystemName", "lfs");

	integer[MAX_PLAYERS] = getGlobalNumber(L, "maxPlayers");
	integer[PZ_LOCKED] = getGlobalNumber(L, "pzLocked", 60000);
	integer[DEFAULT_DESPAWNRANGE] = Pokemon::despawnRange = getGlobalNumber(L, "deSpawnRange", 2);
	...
	integer[RATE_POKEDEX] = getGlobalNumber(L, "ratePokedex", 5);
	...
	integer[MAX_POKEMON_LEVEL] = getGlobalNumber(L, "maxPokemonLevel", 100);
