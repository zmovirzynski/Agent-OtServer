...
struct Position
{
	...
	static int32_t getDistance(const Position& p1, const Position& p2) {
		return std::max<int32_t>(
			std::max<int32_t>(
				std::abs(Position::getDistanceX(p1, p2)),
				std::abs(Position::getDistanceY(p1, p2))
			),
			std::abs(Position::getDistanceZ(p1, p2))
		);
	}
