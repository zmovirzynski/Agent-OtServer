...
ReturnValue Actions::internalUseItem(Player* player, const Position& pos, uint8_t index, Item* item, bool isHotkey)
{
	...
	if (g_moduleCallback->executeUseItem(player, item, pos, nullptr, pos))
		return RETURNVALUE_NOERROR;
                        ...
                        uint8_t toStackPos, Item* item, bool isHotkey, Creature* creature/* = nullptr*/)
{
	...
	if (creature) {
		...
		thing = g_game.internalGetThing(player, toPos, toStackPos, 0, STACKPOS_USETARGET);
	}

	bool moduleRet = g_moduleCallback->executeUseItem(player, item, fromPos, thing, toPos);
	if (!action && !moduleRet) {
