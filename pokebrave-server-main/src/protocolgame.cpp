	...
	if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		if (!g_game.placeCreature(player, player->getLoginPosition())) {
			if (!g_game.placeCreature(player, player->getPokemonCenterPosition(), false, true)) {
				disconnectClient("Pokemon Center position is wrong. Contact the staff.");
				return;
			}
		}
...
void ProtocolGame::logout(bool displayEffect, bool forced)
{
	...
	if (!player->isRemoved()) {
		if (!forced) {
			...
			if (!g_moduleCallback->executeOnLogout(player)) {
				return;
			}
		}
...
void ProtocolGame::sendUpdatePokemon(Pokemon* pokemon)
{
	NetworkMessage msg;
	msg.addByte(0x4C);
	msg.add<uint32_t>(pokemon->getID());
	msg.add<uint32_t>(pokemon->getLevel());
	msg.add<uint8_t>(pokemon->getGender());
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendBasicData()
{
	...
	msg.add<uint16_t>(player->getGroup()->id);
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	if (creatureType == CREATURETYPE_POKEMON) {
		...
		if (g_config.getBoolean(ConfigManager::POKEMON_FEATURE_LEVEL)) {
			msg.add<uint32_t>(pokemon->getLevel());
		}
		if (g_config.getBoolean(ConfigManager::POKEMON_FEATURE_GENDER)) {
			msg.add<uint8_t>(pokemon->getGender());
		}
	}
}
