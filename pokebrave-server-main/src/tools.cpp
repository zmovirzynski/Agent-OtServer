...
std::string getPokemonGenderName(PokemonGender_t gender)
{
	switch (gender)
	{
	case POKEMON_GENDER_UNDEFINED:
		return "undefined";
		break;
...
std::string getPokemonNatureName(PokemonNature_t nature)
{
	switch (nature)
	{
...
std::string getPokemonTypeName(PokemonType_t type)
{
	switch (type)
	{
...
std::string getPokemonStats(PokemonValues& values)
{
	uint16_t statsPoints = values.attack + values.defense + values.health +
		values.specialAttack + values.specialDefense + values.speed;

	if (statsPoints <= 90) {
		return "ok";
	}
	else if (statsPoints >= 91 && statsPoints <= 120) {
		return "good";
	}
	else if (statsPoints >= 121 && statsPoints <= 150) {
		return "great";
	}
	else if (statsPoints >= 151 && statsPoints <= 186) {
		return "amazing";
	}
	return "none";
}
...
const char* getReturnMessage(ReturnValue value)
{
	switch (value) {
		...
		case RETURNVALUE_YOURPROFESSIONCANNOTUSETHISSPELL:
			return "You have the wrong profession to cast this spell.";
		...
		case RETURNVALUE_POKEMONCANNOTEVOLVE:
			return "This pokemon can't evolve.";

		case RETURNVALUE_POKEMONCANNOTEVOLVEWITHTHISSTONE:
			return "This pokemon can't evolve with this stone.";

		case RETURNVALUE_POKEMONEVOLUTIONNOTFOUND:
			return "The evolution of this Pokemon was not found, please contact support.";

		case RETURNVALUE_NOLEVELTOEVOLVEPOKEMON:
			return "You don't have enough level to evolve this pokemon.";
