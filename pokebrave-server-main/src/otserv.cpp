...
JobsScheduler g_jobsScheduler(JOBSSCHEDULER_THREADS);
...
RSA g_RSA;
ModuleCallback* g_moduleCallback;
...
void mainLoader(int, char*[], ServiceManager* services)
{
	...
	g_moduleCallback = new ModuleCallback(g_scripts);

	std::cout << ">> Initialize and loading module callbacks" << std::endl;
	if (!g_moduleCallback->initialize()) {
		startupErrorMessage("Failed to initialize module callbacks");
		return;
	}

	if (!g_moduleCallback->connectCallbacks()) {
		startupErrorMessage("Failed to connect module callbacks");
		return;
	}
	...
	services->add<ProtocolAccount>(static_cast<uint16_t>(g_config.getNumber(ConfigManager::CREATE_ACCOUNT_PORT)));
	services->add<ProtocolCharacter>(static_cast<uint16_t>(g_config.getNumber(ConfigManager::CREATE_CHARACTER_PORT)));
