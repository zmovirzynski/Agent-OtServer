function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if player:getStorageValue(PlayerStorageKeys.delayLargeSeaShell) <= os.time() then
		...
		local msg = ""
		if chance <= 16 then
			doTargetCombat(0, player, COMBAT_NORMALDAMAGE, -200, -200, CONST_ME_NONE)
		...
		player:say(msg, TALKTYPE_POKEMON_SAY, false, player, item:getPosition())
		item:transform(7553)
		item:decay()
		...
		player:say("You have already opened a shell today.", TALKTYPE_POKEMON_SAY, false, player, item:getPosition())
	end
	return true
end
