...
CONDITION_EXHAUST = CONDITION_EXHAUST_COMBAT
...
function getPlayerRequiredSkillTries(cid, skillId) local p = Player(cid) return p and p:getProfession():getRequiredSkillTries(skillId) or false end
...
function doPlayerSetPromotionLevel(cid, level) local p = Player(cid) return p and p:setProfession(p:getProfession():getPromotion()) or false end
...
function doSummonPokemon(cid, name)
	...
	local pokemon = Game.createPokemon(name, position, player)
	if pokemon then
...
getPlayerPromotionLevel = getPromotedProfession
